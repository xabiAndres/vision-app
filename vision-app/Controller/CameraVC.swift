    ...
    func resultsMethod(request: VNRequest, error: Error?) {
        ...
        for clasification in results {
            if clasification.confidence < 0.5 {
                ...
                synthesisSpeech(fromString: unknownObjectMessage)
                self.confidenceLbl.text = ""
                break
            } else {
                let identification = clasification.identifier
                let confidence = Int(clasification.confidence * 100)
                self.identificationLbl.text = identification
                self.confidenceLbl.text = "CONFIDENCE: \(confidence)%"
                let completeSentence = "This looks like a \(identification) and I'm \(confidence) percent sure"
                synthesisSpeech(fromString: completeSentence)
                break
            }
        }
    }
    
    func synthesisSpeech(fromString string: String) {
